...
// protein (4) is active and etot (8) is active
__attribute__((always_inline)) void
wrap(int il, float transform[3][4][WGSIZE], const int natlig, const int natpro,
     const Atom *restrict protein, const Atom *restrict ligand,
     const FFParams *__restrict forcefield, const int group,
  ...
  for (int l = 0; l < WGSIZE; l++) {
    lpos_x[l] = transform[0][3][l] + l_atom.x * transform[0][0][l] +
                l_atom.y * transform[0][1][l] + l_atom.z * transform[0][2][l];
    lpos_y[l] = transform[1][3][l] + l_atom.x * transform[1][0][l] +
                l_atom.y * transform[1][1][l] + l_atom.z * transform[1][2][l];
    lpos_z[l] = transform[2][3][l] + l_atom.x * transform[2][0][l] +
                l_atom.y * transform[2][1][l] + l_atom.z * transform[2][2][l];
  }
  ...
  do {
    ...
    // [activity] may require field sensitivity but this is constant
    ...
    const float elcdst =
        (p_params.hbtype == HBTYPE_F && l_params.hbtype == HBTYPE_F) ? 4.f
                                                                     : 2.f;
    const float elcdst1 =
        (p_params.hbtype == HBTYPE_F && l_params.hbtype == HBTYPE_F) ? 0.25f
                                                                     : 0.5f;
    const int type_E =
    ...
    const float p_hphb = p_params.hphb * (phphb_ltz && lhphb_gtz ? -1.f : 1.f);
    const float l_hphb = l_params.hphb * (phphb_gtz && lhphb_ltz ? -1.f : 1.f);
    ...
    for (int l = 0; l < WGSIZE; l++) {
      ...
      // [activity] lpos are all constant
      // [activity] x, y, distij are all active
      ...
      // [activity] distbb is active
      const float distbb = distij - radij;

      // [activity] may require type analysis but this is constant
      const int zone1 = (distbb < 0.f);

      // Calculate steric energy
      // The RHS of the mul is constant
      etot[l] += (1.f - (distij * r_radij)) * (zone1 ? 2.f * HARDNESS : 0.f);

      // Calculate formal and dipole charge interactions
      float chrg_e = chrg_init * ((zone1 ? 1.f : (1.f - distbb * elcdst1)) *
...
void fasten_main(const int natlig, const int natpro,
...

  {
    ...
    do {
      ...
      wrap(il, transform, natlig, natpro, protein, ligand, forcefield, group,
           etot);
