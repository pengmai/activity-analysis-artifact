...
__global__ void xs_lookup_kernel_baseline(Inputs in, SimulationData GSD )
{
	...
	double macro_xs_vector2[5] = {0};
		calculate_macro_xs(
			...
			#endif
    #else
  double *d_concs = (double*)malloc(GSD.length_concs * sizeof(double));
  double *d_uae = (double *)malloc(GSD.length_unionized_energy_array * sizeof(double));
  #if ALL_ACTIVE
	// Everything active
	__enzyme_autodiff((void*)calculate_macro_xs,
			...
			enzyme_dup, GSD.concs, d_concs,        // Flattened 2-D array with concentration of each nuclide in each material
			enzyme_dup, GSD.unionized_energy_array, d_uae, // 1-D Unionized energy array
