...
CUDA_PATH=os.getenv("CUDA_PATH", "/usr/local/cuda-12.6")
    ...
    for size in sizes:
        ...
        for i in range(runs):
            if VERIFY == "yes":
                ...
                res.append(os.popen(f"CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES={DEVICE} {CUDA_PATH}/bin/ncu --nvtx -k ApplyMaterialPropertiesAndUpdateVolume_kernel --target-processes all ./lulesh -s " + str(size)).read().strip())
